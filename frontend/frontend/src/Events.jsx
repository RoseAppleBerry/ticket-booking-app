...
function Events() {
  ...
  const fetchEvents = async () => {
    try {
      const response = await fetch("app-mongo.s5im9.mongodb.net/event/all", {
        headers: {
...
const handleBookEvent = async (eventId) => {
    try {
      ...
      const response = await fetch(`app-mongo.s5im9.mongodb.net/ticket/user`, {
        headers: {
          Authorization: `${token}`,
          "Content-Type": "application/json",
        },
      ...
      if (isAlreadyBooked) {
        ...
        const bookResponse = await fetch(`app-mongo.s5im9.mongodb.net/ticket`, {
          method: "POST",
          headers: {
            Authorization: `${token}`,
            "Content-Type": "application/json",
